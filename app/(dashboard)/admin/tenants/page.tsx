...
export default function TenantsPage() {
  ...
  const handleActivate = async (tenantId: string) => {
    try {
      ...
      setTenants((prev) => prev.map((tenant) => (tenant.id === tenantId ? updated : tenant)))
    } catch (e) {
      setError(getErrorMessage(e))
    }
  }
    ...
    <div className="flex-1 p-6 md:p-8 max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-muted-foreground">Manage all registered organizations and accounts</p>
        </div>
      </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-foreground">
                ₦{(tenants.reduce((sum, t) => sum + (t.monthly_revenue ?? 0), 0) / 1000).toFixed(0)}K
              </p>
            </div>
        ...
        {filteredTenants.map((tenant) => (
          <Card key={tenant.id} className="overflow-hidden shadow-card hover:shadow-card-hover transition-all">
              ...
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-semibold text-foreground">{tenant.total_users ?? 0}</p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-semibold text-foreground">{tenant.total_branches ?? 0}</p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
            ...
            <div className="px-6 py-4 bg-muted/50 border-t border-border flex gap-2">
              <Button size="sm" variant="outline" className="flex-1" onClick={() => router.push(`/admin/tenants/${tenant.id}`)}>
                <Eye className="w-4 h-4 mr-2" />
                View
              </Button>
              {tenant.status === 'suspended' ? (
                <Button size="sm" onClick={() => handleActivate(tenant.id)} className="flex-1 bg-green-600 hover:bg-green-700 text-white">
                  <CheckCircle className="w-4 h-4 mr-2" />
                  Activate
                </Button>
              ) : (
                <Button size="sm" onClick={() => handleSuspend(tenant.id)} className="flex-1 bg-orange-600 hover:bg-orange-700 text-white">
                  <XCircle className="w-4 h-4 mr-2" />
                  Suspend
                </Button>
              )}
              <Button size="sm" variant="outline" onClick={() => handleDelete(tenant.id)} className="text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20">
